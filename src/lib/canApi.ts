 ...
 *
 * 和 can-web 的同名文件**导出同样的三个东西、同样的签名**（`api`、`apiFetch`、
 * `unwrapList`），这不是巧合：预约看板、ATIS 制作器和管制员概览是从 can-web
 * 原样搬过来的，接口对齐了，那三个岛屿就是逐字复制而不是重写 —— 重写它们意味
 * 着重新犯一遍 `unwrapList` 下面记着的那三个错。
 *
 ...
 * `/api/v1/super/redemption` 给的是 `data: {redemptions: [...]}`。
 *
 ...
 * 页面坏了，而不是像响应读错了。**ATC 预约看板正是这么上线的三个页面之一。**
 *
 * 所以键名是必填而不是可选，两种形状都能拿到数组。认不出来的形状返回空列表而
 * 不是抛：一块空看板还能救，一次渲染中途死掉救不回来。
 */
