...
**没搬：** SUP 的那几个入口（活动管理、奖品管理、处理结果公示），以及晋升那条流程
的三页本身（花名册、晋升、晋升审批）—— 搬过来的只有它们的**菜单**，见下一节。前者
不是「管制员自己的那四件事」：活动、奖品、反馈都是网络的管理面，飞行员那一侧也用得
到其中几个。

**那七个页面后来自己搬了一次，但不是搬到这里。** 它们现在是 **can-portal**
（`portal.ceruleanavi.net`，教员与管理门户）的，不再在主站上 —— 主站的 `/instr` 和
`/super` 各自只剩一个转发页。对这个仓库来说变的只有链接的**主机**：
`src/lib/nav.ts` 里那四条从 `webUrl()` 改成了 `portalUrl()`，前缀和路径一个字都没
动（门户刻意留着 `/instr` 和 `/super`，因为它们编码的是评级边界）。菜单该不该在这
里、对谁出现，那道题的答案没有变。
...
「模拟机场景」是 can-portal 的 SweatBox 场景生成器（`/instr/sweatbox`）：勾席位、定流量，
...
**收起来的是菜单，不是页面。** 那四条指向的是 **can-portal** 的 `/instr/roster`、
`/instr/promote`、`/instr/sweatbox` 和 `/super/promotions`；拦住普通管制员的是那个
站自己的评级中间件（教员 8 / 监理 11 / ADM 12）和 can-api 每条路由自己的守卫。这里的门槛只是为了不给人看一组他点下去必然被拒的链接 —— 把 8 改小不会放开任何东
...
**can-web 的 `/controllers/*` 不再提供这四个页面，只剩一个转发页**
（`can-web/src/pages/controllers/[...path].astro`），把旧地址 301 到这里，其中
`/controllers/atc-reservations` → `/reservations` 是唯一改了名的一条。所以从今天
起**这个站是唯一的权威**，不再有「两边都活着」的过渡期 —— 主站上那些页面、它们
的岛屿和 `src/content/rules/` 都删掉了。

那份转发是必需的、也是脆的：它是一张手写的对应表，没有任何测试盯着它。这里加一
个新页面时，如果 can-web 上有对应的旧地址，顺手去那张表里补一行。
